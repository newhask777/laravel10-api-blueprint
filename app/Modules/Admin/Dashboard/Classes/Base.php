...
//        dd($this->vars);
    ...
    private function getMenu()
    {
        return Menu::make('menuRenderer', function ($m){
            foreach (MenuModel::menuByType(MenuModel::MENU_TYPE_ADMIN)->get() as $item)
            {
                ...
                if ($item->parent == 0)
                {
                    $m->add($item->title, $path)->id($item->id)->data('perrmissions', []);
                }
                else
                {
                    if ($m->find($item->parent))
                    {
                        $m->find($item->parent)->add($item->title, $path)->id($item->id)->data('perrmissions', []);;
                    }
                }
    ...
    {
        $routes = Route::getRoutes()->getRoutes(); # return all routes
